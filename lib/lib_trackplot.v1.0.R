...
.plot_track = function(summary_list, chr, start, end, draw_gene_track = FALSE, gene_model = NULL, gtf = FALSE, query_ucsc = NULL, build = "hg19", col = "gray70", autoscale = TRUE, fixscale = 0, txname = NULL, genename = NULL, gene_width = 2, scale_width = 1, plot_axis = TRUE, u_fount = 0.6, track_names = NULL, track_names_pos = 0, regions = NULL, region_width = 1, collapse_txs = collapse_tx, boxcol = "#192A561A", boxcolalpha = 0.2, show_ideogram = FALSE){
  ...
  if (length(fixscale) > 0) {
    if (length(fixscale) == length(summary_list)) {
      plot_height = fixscale
    } else {
      plot_height = rep(fixscale[1], length(summary_list))
    }
  }
